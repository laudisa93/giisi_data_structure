...
public class BinaryTree implements IBinaryTree  {
    ...
    public void addRoot(Object newRoot) throws Exception {
        if (this.isEmpty()) {
            this.root = new Node(newRoot, null);

        } else {
            throw new Exception("Cannot add a root: tree is not empty!!");
        }
    ...
    public Object replace(INode nodeToChange, Object newElement) throws InvalidNodeException {
        Node verifiedNode = checkPosition(nodeToChange);
    ...
    public boolean isInternal(INode nodeToCheck) throws InvalidNodeException {
        Node verifiedNode = checkPosition(nodeToCheck);
    ...
    public Node parent(INode nodeToCheck) throws InvalidNodeException {
        Node verifiedNode = checkPosition(nodeToCheck);
        Node parentNode = verifiedNode.getParent();
        if (parentNode == null) {
    ...
    public Iterator children(INode nodeToParent) throws InvalidNodeException {
        Node verifiedNode = checkPosition(nodeToParent);
    ...
    public Node left(INode nodeToCheck) throws InvalidNodeException, BoundaryViolationException {
        Node verifiedNode = checkPosition(nodeToCheck);
    ...
    public Node right(INode nodeToCheck) throws InvalidNodeException, BoundaryViolationException {
        Node verifiedNode = checkPosition(nodeToCheck);
    ...
    protected Node checkPosition(INode nodeToCheck)
    ...
    public void insertNodeLeft(INode father, Object newElement) throws InvalidNodeException {
        Node parentNode = this.checkPosition(father);
    ...
    public Object remove(INode node) throws InvalidNodeException {
        Node nodeToRemove = this.checkPosition(node);
        ...
        if (this.isRoot(nodeToRemove)) {
            ...
            Node parentOldNode = nodeToRemove.getParent();
            ...
            if (onlyChild != null) {
                onlyChild.setElement(parentOldNode);
            }
        }
        // this.size--;
    ...
    public void attach(INode nodeAttaching, BinaryTree T1, BinaryTree T2)
            throws InvalidNodeException {
        Node vv = checkPosition(nodeAttaching);
